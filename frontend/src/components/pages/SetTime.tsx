...
const SetTime = () => {
  ...
  const updateFinishDate = (
    ...
    setFinishDate.setHours(
      setFinishDate.getHours(),
      setFinishDate.getMinutes() + time,
      setFinishDate.getSeconds(),
      setFinishDate.getMilliseconds()
    );
    ...
    UpdateTaskOngoing({
      ...
      (res) => {
        if (newSelectedArray.length == 0)
          history.push("/TaskView?userRole=CHEF");
        else updateSteamer(dbTaskId, selected);
      },
      (err) => {
  ...
  return (
    ...
    >
      <div
        style={{ border: "3px solid #683830", borderRadius: "10px" }}
        ...
        <div className="w-auto sm:w-auto md:w-auto lg:w-auto xl:w-auto max-w-screen-md text-center">
          <table className="table-fixed">
            <tr>
              <td>
                <div
                  className="flex text-xl font-bold w-full text-center underline mt-3 ml-2"
                  style={{ paddingBottom: "15px" }}
                >
                  <div style={{ textAlign: "left" }}>
                    {status === "ONGOING" || status === "TIMEUP"
                      ? "เสิร์ฟอาหารหรือเพิ่มเวลา"
                      : "จับเวลา"}
                  </div>
                </div>
              </td>
              <td style={{ textAlign: "right" }}>
                <div className="font-thin text-base underline mr-4">
                  <a
            ...
            <tr>
              ...
              <td className="w-1/4 px-2 py-0">
                <div className="flex flex-wrap">
                  <div
                    className="px-2 py-2 m-1 font-bold text-base rounded-md w-full block text-center"
                  ...
                  <div
                    className="px-2 py-2 m-1 text-white font-bold text-base rounded-md w-full block text-center"
                    style={{
                      display:
                        status === "ONGOING" || status === "TIMEUP"
                          ? ""
